...
class TracyDebugger extends \ProcessWire\WireData implements \ProcessWire\Module, \ProcessWire\ConfigurableModule {
 ...
 require_once(\ProcessWire\wire('files')->compile(\ProcessWire\wire("config")->paths->root . 'site/modules/TracyDebugger' . '/includes/ShortcutMethods.php',array('includes'=>true,'namespace'=>true,'modules'=>false,'skipIfNamespace'=>false)));
        ...
        $this->wire()->addHookAfter('Wire::trackException', function($event) {
            ...
            if(!$exception instanceof \ProcessWire\WireException && !$exception instanceof \ProcessWire\WireException) {
                throw $exception;
            }
        });
 ...
 require_once(\ProcessWire\wire('files')->compile(\ProcessWire\wire("config")->paths->root . 'site/modules/TracyDebugger' . '/includes/ConsoleSnippets.php',array('includes'=>true,'namespace'=>true,'modules'=>false,'skipIfNamespace'=>false)));
    ...
    /**
     * Hook after \ProcessWire\ProcessWire::ready()
     *
 ...
 require_once(\ProcessWire\wire('files')->compile(\ProcessWire\wire("config")->paths->root . 'site/modules/TracyDebugger' . '/includes/user-bar/UserBar.php',array('includes'=>true,'namespace'=>true,'modules'=>false,'skipIfNamespace'=>false)));
    ...
    /**
     * Hook before \ProcessWire\ProcessWire::finished
     *
 ...
 require_once(\ProcessWire\wire('files')->compile(\ProcessWire\wire("config")->paths->root . 'site/modules/TracyDebugger' . '/includes/PwApiData.php',array('includes'=>true,'namespace'=>true,'modules'=>false,'skipIfNamespace'=>false)));
    ...
    public function getModuleConfigInputfields(array $data) {
        ...
        if($this->wire('input')->post->clearEmailSent) {
            ...
            if (!isset($removed) || !$removed) \ProcessWire\wire()->error( \ProcessWire\__('No file to remove'));
            else \ProcessWire\wire()->message(\ProcessWire\__("email-sent file deleted successfully"));
        }
        ...
        $f->attr('name', 'email');
        $f->label = \ProcessWire\__('Email for production errors', \ProcessWire\wire("config")->paths->root . 'site/modules/TracyDebugger/TracyDebugger.module.php');
        $f->description = \ProcessWire\__('Receive emails at this address when an error occurs in production mode.', \ProcessWire\wire("config")->paths->root . 'site/modules/TracyDebugger/TracyDebugger.module.php');
        $f->columnWidth = 40;
        ...
        $fieldset->label = \ProcessWire\__('Snippet Runner panel', \ProcessWire\wire("config")->paths->root . 'site/modules/TracyDebugger/TracyDebugger.module.php');
        $wrapper->add($fieldset);
        ...
        $f->description = \ProcessWire\__('This path will be checked for snippets.', \ProcessWire\wire("config")->paths->root . 'site/modules/TracyDebugger/TracyDebugger.module.php');
        $f->notes = \ProcessWire\__('Neither of these directories exist by default so you will need to create them.', \ProcessWire\wire("config")->paths->root . 'site/modules/TracyDebugger/TracyDebugger.module.php');
        ...
        $f->label = \ProcessWire\__('Restricted Roles', \ProcessWire\wire("config")->paths->root . 'site/modules/TracyDebugger/TracyDebugger.module.php');
        ...
        $f->notes = \ProcessWire\__('This can be useful if you use the User system to store frontend "members" and the system has a lot of users.', \ProcessWire\wire("config")->paths->root . 'site/modules/TracyDebugger/TracyDebugger.module.php');

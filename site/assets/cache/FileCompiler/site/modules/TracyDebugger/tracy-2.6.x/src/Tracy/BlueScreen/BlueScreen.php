...
class BlueScreen
{
	...
	{
		$this->collapsePaths[] = preg_match('#(.+/vendor)/tracy/tracy/src/Tracy/BlueScreen$#', strtr(\ProcessWire\wire("config")->paths->root . 'site/modules/TracyDebugger/tracy-2.6.x/src/Tracy/BlueScreen', '\\', '/'), $m)
			? $m[1]
			: \ProcessWire\wire("config")->paths->root . 'site/modules/TracyDebugger/tracy-2.6.x/src/Tracy/BlueScreen';
	}
	...
	public function render(\Throwable $exception): void
	{
		if (Helpers::isAjax() && session_status() === PHP_SESSION_ACTIVE) {
			ob_start(function () {});
			$this->renderTemplate($exception, \ProcessWire\wire("config")->paths->root . 'site/modules/TracyDebugger/tracy-2.6.x/src/Tracy/BlueScreen' . '/assets/content.phtml');
			$contentId = $_SERVER['HTTP_X_TRACY_AJAX'];
			$_SESSION['_tracy']['bluescreen'][$contentId] = ['content' => ob_get_clean(), 'time' => time()];

		} else {
			$this->renderTemplate($exception, \ProcessWire\wire("config")->paths->root . 'site/modules/TracyDebugger/tracy-2.6.x/src/Tracy/BlueScreen' . '/assets/page.phtml');
		}
	}
	...
	public function renderToFile(\Throwable $exception, string $file): bool
	{
		if ($handle = @fopen($file, 'x')) {
			...
			$this->renderTemplate($exception, \ProcessWire\wire("config")->paths->root . 'site/modules/TracyDebugger/tracy-2.6.x/src/Tracy/BlueScreen' . '/assets/page.phtml', false);
			ob_end_flush();
			ob_end_clean();
			fclose($handle);
			return true;
	...
	private function renderTemplate(\Throwable $exception, string $template, $toScreen = true): void
	{
		...
		$css = array_map('file_get_contents', array_merge([
			\ProcessWire\wire("config")->paths->root . 'site/modules/TracyDebugger/tracy-2.6.x/src/Tracy/BlueScreen' . '/assets/bluescreen.css',
			\ProcessWire\wire("config")->paths->root . 'site/modules/TracyDebugger/tracy-2.6.x/src/Tracy/BlueScreen' . '/../Toggle/toggle.css',
			\ProcessWire\wire("config")->paths->root . 'site/modules/TracyDebugger/tracy-2.6.x/src/Tracy/BlueScreen' . '/../TableSort/table-sort.css',
			\ProcessWire\wire("config")->paths->root . 'site/modules/TracyDebugger/tracy-2.6.x/src/Tracy/BlueScreen' . '/../Dumper/assets/dumper.css',
